...
def get_datasets_Emotions(config):
    
    train_list = []
    image_list = []
    mediapipe_list = []

    for dirpath, dirnames, images in os.walk(config.dataset.Emotion_Recognition_path):
        for image in images:
    ...
    dataset = EmotionDataset(train_list, config, test=False)
    return dataset